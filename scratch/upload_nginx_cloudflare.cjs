...
// The optimized Nginx config for Cloudflare CDN
const nginxConfig = `server {
    ...
    # ── Uploaded Product Images (CDN cacheable for 90 days) ─────────────────────
    location /uploads/ {
    ...
    # ── Hashed JS/CSS/Font assets from Vite (cache 1 year — filenames have hash) ─
    ...
    # ── SEO / SSR routes via Node.js backend ─────────────────────────────────────
...
const localPath = '/tmp/gazisports24_nginx.conf';
fs.writeFileSync('./scratch/gazisports24_nginx.conf', nginxConfig);
console.log('📝 Config written locally to scratch/gazisports24_nginx.conf');

// Now upload via SFTP and apply
const conn = new Client();
conn.on('ready', () => {
  console.log('✅ SSH Connected! Uploading Nginx config via SFTP...');

  conn.sftp((err, sftp) => {
    if (err) { console.error('SFTP error:', err); conn.end(); return; }

    const remotePath = '/etc/nginx/sites-enabled/gazisports24.com';
    const writeStream = sftp.createWriteStream(remotePath);

    writeStream.on('close', () => {
      console.log('✅ Config uploaded! Testing and reloading Nginx...\n');

      const testCmd = 'nginx -t && systemctl reload nginx && echo "✅ Nginx reloaded!" || echo "❌ Nginx config error!"';
      conn.exec(testCmd, (err2, stream) => {
        if (err2) { console.error(err2); conn.end(); return; }
        stream.on('close', (code) => {
          console.log(`\nExit code: ${code}`);
          if (code === 0) console.log('\n🎉 Cloudflare-optimized Nginx config is LIVE!');
          conn.end();
